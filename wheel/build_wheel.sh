...
if [[ -z "$BUILD_PYTHONLESS" ]]; then
    ...
    if [[ $LIBTORCH_VARIANT = *"shared"* ]]; then
        ...
        elif [[ -x "$(pwd)/any_wheel/torch/lib/libiomp5.dylib" ]]; then
    ...
    zip -rq "$PYTORCH_FINAL_PACKAGE_DIR/libtorch-macos-$LIBTORCH_VARIANT-$CMAKE_OSX_ARCHITECTURES-$PYTORCH_BUILD_VERSION.zip" libtorch
    cp "$PYTORCH_FINAL_PACKAGE_DIR/libtorch-macos-$LIBTORCH_VARIANT-$CMAKE_OSX_ARCHITECTURES-$PYTORCH_BUILD_VERSION.zip" \
       "$PYTORCH_FINAL_PACKAGE_DIR/libtorch-macos-$LIBTORCH_VARIANT-$CMAKE_OSX_ARCHITECTURES-latest.zip"
    popd
fi
